...
function readToken(ast, token) {
  ...
  if (token.type === "operator" && token.value === "{") {
    ...
    if (
      objectOrBlock.nodes.length > 0 &&
      objectOrBlock.nodes[0].type !== NODES.KEYWORD
    ) {
...
function writeCall(node) {
  ...
  if (calleeName === "fn") {
    ...
    if (node.nodes.length >= 3) {
      ...
      if (spreadNode.type === NODES.SYMBOL && spreadNode.value === "..") {
        prelude =
          "var " +
          escapeSymbol(node.nodes[node.nodes.length - 2].value) +
  ...
  if (calleeName === "get") {
    if (node.nodes.length !== 2) {
      throw new Error(
        'builtin "get" takes exactly 2 params: key and target, got: ' +
          prettyPrint(node)
      );
    }
  ...
  if (calleeName === "set" || calleeName === "=") {
    if (node.nodes.length !== 2) {
      throw new Error(
        'builtin "set" takes exactly 2 params: name and value, got: ' +
          prettyPrint(node)
      );
    }
    return writeNode(node.nodes[0]) + " = " + writeNode(node.nodes[1]);
  ...
  if (calleeName === "let") {
    if (node.nodes.length !== 2) {
      throw new Error(
        'builtin "let" takes exactly 2 params: name and value, got: ' +
          prettyPrint(node)
      );
    }
    if (node.nodes[0].type !== NODES.SYMBOL) {
      throw new Error(
        'builtin "let" need a symbol as first parameter, got: ' +
          prettyPrint(node)
      );
    }
  ...
  if (calleeName === "delete") {
    if (node.nodes.length !== 2) {
      throw new Error(
        'builtin "delete" takes exactly 1 param: target, got: ' +
          prettyPrint(node)
      );
    }
  ...
  if (calleeName === "if") {
    if (node.nodes.length < 2 || node.nodes.length > 3) {
      throw new Error(
        'builtin "if" a minimum of 2 and a maximum of 3 arguments (condition then else), got: ' +
          prettyPrint(node)
      );
    }
    ...
    var elseBody =
      "{return " +
      (node.nodes[2] ? writeNode(node.nodes[2]) : "undefined") +
      ";}";
    return wrapInIife(
      "if (" + writeNode(node.nodes[0]) + ") " + thenBody + " else " + elseBody
    );
  }
  ...
  if (calleeName === "cond") {
    if (node.nodes.length % 2 !== 0) {
      throw new Error(
        'builtin "cond" needs a pair amount of items, (condition result)*, got: ' +
          prettyPrint(node)
      );
    }
    var result = [];
    for (var i = 0; i < node.nodes.length; i++) {
      if (i % 2 === 1) {
        ...
        if (
          node.nodes[i - 1].type === NODES.KEYWORD &&
          node.nodes[i - 1].value === "else"
        ) {
          result.push(ifBody);
        } else {
  ...
  if ("+-*/".indexOf(calleeName) > -1) {
    if (node.nodes.length < 2) {
      throw new Error(
        'builtin "' +
          calleeName +
          '" needs a minimum of 2 arguments, got: ' +
          prettyPrint(node)
      );
    }
  ...
  return (
    writeNode(node.callee) + "(" + node.nodes.map(writeNode).join(", ") + ")"
  );
}
...
function writeNode(node) {
  ...
  if (node.type === NODES.OBJECT) {
    if (node.nodes.length % 2 !== 0) {
      throw new Error(
        "object literal contains an odd number of elements, need a value for each key" +
          "\ngiven: " +
  ...
  if (node.type === NODES.BLOCK) {
    return (
      ...
      (node.nodes.length > 1 ? ";" : "") +
      "return " +
      writeNode(node.nodes[node.nodes.length - 1]) +
      ";}).call(this)"
    );
  }
...
function compile(code) {
  ...
  core.forEach(function(coreFnName) {
    if (result.indexOf(coreFnName) > -1) {
      prelude +=
        "var " + coreFnName + ' = require("eth/core").' + coreFnName + ";";
    }
  }, core.keys(core));
...
function prettyPrint(ast) {
  function prettyPrintNode(node) {
    ...
    if (node.type === NODES.CALL) {
      return (
        prettyPrintNode(node.callee) +
        "(" +
        node.nodes.map(prettyPrintNode).join(" ") +
        ")"
      );
    }
    if (node.type === NODES.REF) {
